...
Runtime code and executable tests are the reproducible source of truth. Manual live-provider
verification is point-in-time evidence; repeating it requires operator-supplied temporary
credentials and external service availability. Risk describes residual verification risk, not a
claim of production readiness.

| Requirement | Implementation | Tests | Verification procedure | Status | Risk |
|---|---|---|---|---|---|
...
| Pinecone dense/hybrid FastAPI runtime | `api/runtime.py:create_api_retriever`, `DenseRetrievalService`, `HybridRetrievalService`, `PineconeSdkGateway`; `RETRIEVAL_MODE=pinecone_hybrid` | `test_runtime_retrieval.py`, `test_dense_retrieval.py`, `test_sparse_hybrid.py` are the reproducible CI authority | Routine `check-index` plus Viewer/Admin/RBAC queries in `dense-retrieval-design.md` | Implemented; manual live verification completed | Medium — future re-verification depends on temporary credentials and external service availability |
...
| LangSmith complete safe tracing | `observability/tracing.py`, graph/node/research/LLM/MCP spans and bounded shutdown flush | hierarchy, sanitization, concurrency, graph tracing, MCP and failure-matrix tests are the reproducible CI authority | Live root/children and privacy inspection steps in `langsmith-tracing-design.md` | Implemented; manual live verification completed | Medium — future re-verification depends on temporary credentials and external service availability |
| Authentication and Viewer/Analyst/Admin RBAC | `security`, API dependencies, strict JWT/token/password settings | authentication, authorization, IDOR/session and role-acceptance suites | Login as all roles; denied and allowed tool/document flows | Implemented | Low — local identities, not enterprise IdP |
...
| Repository review readiness | README, contributing guide, architecture and design documentation | link checker, repository scans, CI quality gates | Review documented commands, clean Git scope, links, and CI status | Implemented | Low — external repository access remains operator-controlled |
...
was found absent from runtime. The final Docker lifecycle was verified locally. Manual live
Pinecone and LangSmith verification has been completed; future re-verification remains dependent
on temporary credentials and external service availability.
