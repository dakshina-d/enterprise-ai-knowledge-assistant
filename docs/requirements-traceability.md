...
| Streaming responses | Must | FastAPI; [event design](event-stream-design.md), [chat/SSE design](fastapi-chat-sse-design.md), [ADR 0004](adr/0004-server-sent-events.md) | SSE ordering, redaction, disconnect and terminal-event tests | Implemented | Native POST stream; durable replay and Streamlit remain planned |
...
| FastAPI | Must | Backend; [API contracts](api-contracts.md), [chat/SSE design](fastapi-chat-sse-design.md) | OpenAPI/endpoint integration and startup tests | Implemented | Health, auth, JSON chat, and native SSE |
| Async APIs | Must | Backend/graph; [architecture](architecture.md), [chat/SSE design](fastapi-chat-sse-design.md) | Request, streaming, lifecycle and cancellation tests | Implemented | Shared process runtime; distributed state remains planned |
...
| Input validation | Must | FastAPI/graph; [API contracts](api-contracts.md), [graph design](graph-design.md) | Boundary/size/schema and injection contract tests | Implemented | Strict chat body and safe 422 response |
