...
Graph 1.2 implements bounded research and a constrained MCP route as a compiled `StateGraph`. MCP service data crosses a local official-SDK client/server boundary and is converted immediately into application-owned models; the host authorizes before opening the in-memory transport. Durable distributed workers, remote MCP transport/OAuth, human approval, reranking, chat transport, SSE, and Streamlit are not implemented.
