...
Graph 1.2 implements bounded research as a compiled `StateGraph`: validated tasks fan out through executable `Send` objects, reducer-backed results aggregate deterministically, and later bounded rounds investigate validated gaps. The catalog, budgets, restricted analysis, evidence ledger, coverage/conflict checks, grounded citations, and privacy-safe LangSmith adapter remain application-owned. Durable distributed workers, MCP, human approval, reranking, chat transport, SSE, and Streamlit are not implemented.
